...
type RequestSaveOptionsWeb struct {
	...
	ShowSiteLogo         bool   `json:"showSiteLogo"`
	DateFormat           string `json:"dateFormat"`
	AccentColorTags      bool   `json:"accentColorTags"`
...
func (i ConfigResource) versionCheck(req *restful.Request, resp *restful.Response) {
	...
	if config.Config.Web.UpdateCheck && common.CurrentVersion != "CURRENT" {
		r, err := resty.New().R().
			...
			Get("https://api.github.com/repos/f0r3s1/xbvr/releases/latest")
...
func (i ConfigResource) toggleUseFlareSolverr(req *restful.Request, resp *restful.Response) {
...
func (i ConfigResource) saveOptionsWeb(req *restful.Request, resp *restful.Response) {
	...
	config.Config.Web.ShowSiteLogo = r.ShowSiteLogo
	config.Config.Web.DateFormat = r.DateFormat
	config.Config.Web.AccentColorTags = r.AccentColorTags
